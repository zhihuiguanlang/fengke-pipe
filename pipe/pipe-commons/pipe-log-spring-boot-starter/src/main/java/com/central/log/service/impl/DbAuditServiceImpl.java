 ...
 */
@Slf4j
@ConditionalOnProperty(name = "pipe.audit-log.log-type", havingValue = "db")
...
public class DbAuditServiceImpl implements IAuditService {
    private static final String INSERT_SQL = " insert into sys_logger " +
            " (application_name, class_name, method_name, user_id, user_name, client_id, operation, timestamp,ip_address) " +
            " values (?,?,?,?,?,?,?,?,?)";
    ...
    public void save(Audit audit) {
        this.jdbcTemplate.update(INSERT_SQL
                ...
                , audit.getOperation(), audit.getTimestamp(),audit.getIpAddress());
    }
}
