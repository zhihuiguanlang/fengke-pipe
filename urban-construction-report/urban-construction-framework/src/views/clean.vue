<template>
  <div class="clean">
    ...
    <div class="report elevation-7">
      <p>
        <code class="code">今日</code>，保洁组对
        <code class="code" />舱
        <code class="code" />防火段、监控中心按照保洁标准进行进行保洁，按时按量完成保洁工作。
      </p>
      <p>具体内容包括：</p>
      <p>
        （1）
  ...
  methods: {
    ...
    inquiry() {
      ...
      this.$notice.normal("正在下载");
       window.open(`${http.system}/api-ops/repoert/workorderClean?type=${type}&startTime=${startTime}&endTime=${endTime}`, "_self");
      setTimeout(() => {
        this.wait = false;
      }, 500);
    }
