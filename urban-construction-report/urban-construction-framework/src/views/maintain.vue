<template>
  <div class="maintain">
    ...
    <div class="report elevation-7">
      <p>
        今日共处理设备维修工单
        <code class="code" />个，涉及
        <code class="code" />台设备，全部完成维修，未有设备维修工单逾
        期未完成情况。具体信息如下。
      </p>
      <p>
        （1）工单号
        <code class="code" />， <code class="code" />舱
        <code class="code" />防火段 <code class="code" />设备，由
        <code class="code" />班组到现场进行维修，故障现 象为
        <code class="code" />，经检测，定位故障原因为
        <code class="code" />，采取 <code class="code" />维修 措施后，设备
        <code class="code" />（是/否）恢复正常工作，后续工作：
        <code class="code" />；
      </p>
      <p>
        （2）工单号
  ...
  methods: {
    ...
    inquiry() {
      ...
      this.$notice.normal("正在下载");
       window.open(`${http.system}/api-ops/repoert/workordermMintain?type=${type}&startTime=${startTime}&endTime=${endTime}`, "_self");
      setTimeout(() => {
        this.wait = false;
      }, 500);
    }
