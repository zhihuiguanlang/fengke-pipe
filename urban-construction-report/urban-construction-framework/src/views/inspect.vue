<template>
  <div class="inspect">
    ...
    <div class="report elevation-7">
      <p>
        今日管廊巡检工作按照巡检计划展开，巡检工作完成率
        <code class="code" />%，巡检过程中发现如下问题。
      </p>
      <p>
        （1）
        <code class="code" />舱
        <code class="code" />防火段廊体结构存在
        <code class="code" />问题，
        <code class="code" />（是/否）影响管廊正常运行， 处理结果为
        <code class="code" />，已下发工单，工单号为
        <code class="code" />，待审批结束后由土建组组织维修；
      </p>
      <p>
        （2）
        <code class="code" />舱
        <code class="code" />防火段附属设施
        <code class="code" />设备存在
        <code class="code" />问题，
        <code class="code" />（是/否）影响管廊正 常运行，处理结果为
        <code class="code" />，已下发工单，工单号为
        <code class="code" />，待审批结束后由
        <code class="code" />组组织维修；
      </p>
      <p>
        （3）管廊保护区在
        <code class="code" />位置存在
        <code class="code" />问题，
        <code class="code" />（是/否）影响管廊正常运行， 处理结果为
        <code class="code" />，已通知对方单位并上报管理部门，保持继续监测。
      </p>
    </div>
    <!-- 加载提示框 -->
  ...
  methods: {
    ...
    inquiry() {
      ...
      this.$notice.normal("正在下载");
       window.open(`${http.system}/api-ops/repoert/workorderPatrol?type=${type}&startTime=${startTime}&endTime=${endTime}`, "_self");
      setTimeout(() => {
        this.wait = false;
      }, 500);
    }
